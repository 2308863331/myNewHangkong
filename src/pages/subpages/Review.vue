...
import { getReviewsList, delReview } from '../../api'
import ReviewsEdit from '../../components/ReviewEdit.vue'
...
const addRow = () => {
  if (reviewsForm.value) {
    reviewsForm.value.resetForm(0) // 重置表单，0 表示新增
}
  id.value = 0
...
const editRow = row => {
  if (reviewsForm.value) {
    reviewsForm.value.resetForm(row.id) // 重置表单，0 表示新增
}
  id.value = row.id
  dialogVisible.value = true
}
